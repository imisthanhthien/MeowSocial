  ...
  />
      ...
      <div className="flex justify-between items-center text-sm text-gray-600 mt-2">
        <div className="flex items-center gap-2">
          ...
          >
            <span className={liked ? 'text-red-500 font-bold' : ''}>
              {liked ? '❤️' : '🤍'}
            </span>
            <span className="text-sm text-gray-600">{likeCount} lượt thích</span>
          </button>
        </div>
        <button
...
{commentsLoading ? (
  ...
  <>
    <div
      className={`space-y-3 ${
        showAllComments ? 'max-h-64 overflow-y-auto pr-1' : ''
      }`}
    >
...
<div className="flex justify-between items-start mt-1 w-full">
  ...
  <p className="text-sm text-gray-500">Chưa có bình luận nào.</p>
    ...
    {/* <div className={`space-y-3 ${showAllComments ? 'max-h-64 overflow-y-auto pr-2' : ''}`}>
 {!showAllComments && comments.length > 4 && (
